    ...
    <div>
      ...
      <div className="table-responsive">
        {chatMessageList && chatMessageList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('sender')}>
                  Sender <FontAwesomeIcon icon={getSortIconByFieldName('sender')} />
                </th>
            ...
            <tbody>
              {chatMessageList.map((chatMessage, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {chatMessage.timestamp ? <TextFormat type="date" value={chatMessage.timestamp} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{chatMessage.channelName}</td>
                  <td>
                    {chatMessage.conversation ? (
                      <Link to={`/conversation/${chatMessage.conversation.id}`}>{chatMessage.conversation.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && <div className="alert alert-warning">No Chat Messages found</div>
        )}
      </div>
    </div>
  );
